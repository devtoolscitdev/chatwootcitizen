...
const { accountScopedRoute, isOnChatwootCloud, currentAccount, accountId } = useAccount();
...
const menuItems = computed(() => {
          ...
          ? [
              {
                ...
                to: accountScopedRoute('sidebar_settings_index'),
              },
            ]
          : []),
        {
  ...
  if (dashboardApps.value && dashboardApps.value.length > 0) {
    ...
    if (mainPlacementApps.length > 0) {
      const targetIndex = items.findIndex(
        item => item.name === 'Reports' || item.name === 'Campaigns'
      );
      const insertAt = targetIndex !== -1 ? targetIndex + 1 : 4;
      const formattedMainApps = mainPlacementApps.map(app => ({
        name: `custom_app_${app.id}`,
        label: app.title,
        icon: app.icon || 'i-lucide-layout-grid',
        to: accountScopedRoute('dashboard_app_view', { appId: app.id }),
        activeOn: ['dashboard_app_view'],
      }));
      items.splice(insertAt, 0, ...formattedMainApps);
    }

    // Inserir os demais apps agrupados sob o item 'Apps'
    if (groupedPlacementApps.length > 0) {
      const settingsIndex = items.findIndex(item => item.name === 'Settings');
      items.splice(settingsIndex !== -1 ? settingsIndex : items.length, 0, {
        name: 'Apps',
        label: t('SIDEBAR.APPS'),
        icon: 'i-lucide-layout-grid',
        children: groupedPlacementApps.map(app => ({
          name: `app-${app.id}`,
          label: app.title,
          icon: app.icon || 'i-lucide-layout-grid',
          to: accountScopedRoute('dashboard_app_view', { appId: app.id }),
  ...
  let filteredItems = items.filter(item => {
    ...
    const itemId = nameToIdMap[item.name] || item.name;
    return !hiddenItems.includes(itemId);
  });

  // 2. Reordenar se houver ordem customizada
  if (itemOrder.length > 0) {
    const settingsItem = filteredItems.find(item => item.name === 'Settings');
    const nonSettingsItems = filteredItems.filter(item => item.name !== 'Settings');

    nonSettingsItems.sort((a, b) => {
      const idA = nameToIdMap[a.name] || a.name;
      const idB = nameToIdMap[b.name] || b.name;
      const indexA = itemOrder.indexOf(idA);
      const indexB = itemOrder.indexOf(idB);
    ...
    filteredItems = settingsItem ? [...nonSettingsItems, settingsItem] : nonSettingsItems;
  }
